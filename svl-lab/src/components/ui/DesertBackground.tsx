...
const DesertGraphTheme = ({ children }: { children: React.ReactNode }) => {
  ...
  const cacti = Array.from({ length: 7 }, (_, index) => ({
    id: index,
  ...
  return (
    <div className="relative w-full h-full overflow-hidden bg-gradient-to-b from-orange-300 via-yellow-200 to-orange-100">
      ...
      {/* Children Content */}
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen text-slate-900">
        {children}
      </div>
    </div>
  );
...
export default DesertGraphTheme;
